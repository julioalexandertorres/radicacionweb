...
{ 
    document.getElementById("marco2").src = "./control/comp/comp.html";
...
function pruebamicro(){
    console.log("listones");
    const datos = {
        // Asume que estos datos provienen de tu formulario o interfaz
        dato1: 333,
        dato2: 'Valorpruebadesdeapi'
    };
    fetch('http://34.121.137.69:3000/insertar', {
        method: 'POST',
        headers: {
...
function open_ficha(gid){
    //console.log(gid);
    if(mun=='pueblo_rico'){
        var cod = select_query("select codigo_ant from "+mun+"_catastro.u_terreno where gid = "+gid+"");
        document.getElementById("marco").style.display = 'block';
        document.getElementById("marco").src = "./fichas_prediales/"+mun+"/"+cod+".PDF";
    }
    else if(mun=='apia'){
        var cod = select_query("select codigo from "+mun+"_catastro.u_terreno where gid = "+gid+"");
	document.getElementById("marco").style.display = 'block';
        document.getElementById("marco").src = "./fichas_prediales/"+mun+"/"+cod+".pdf";
    }
    else if(mun=='belen_de_umbria'){
        var cod = select_query("select codigo_ant from "+mun+"_catastro.u_terreno where gid = "+gid+"");
   	document.getElementById("marco").style.display = 'block';
        document.getElementById("marco").src = "./fichas_prediales/"+mun+"/"+cod+".pdf";
    }
   ...
   // document.getElementById("marco").src = "./fichas_prediales/"+mun+"/"+cod+".pdf";
    ...
    //window.open('./fichas_prediales/pueblo_rico/66572010000010001000.pdf', '_blank');
} 

function open_fichasel(gid){
    //console.log(gid);
    var codigo = select_query("select codigo from "+mun+"_catastro.r1 where gid = "+gid+"");
    //console.log(codigo);
    if(mun=='pueblo_rico'){
      var cod = select_query("select codigo_ant from "+mun+"_catastro.u_terreno where codigo = '"+codigo[0][0]+"'");
      document.getElementById("marco").style.display = 'block';
      document.getElementById("marco").src = "./fichas_prediales/"+mun+"/"+cod+".PDF";
    }
    else if(mun=='apia'){
      ...
      document.getElementById("marco").src = "./fichas_prediales/"+mun+"/"+cod+".pdf";
        ...
        var cod = select_query("select codigo from "+mun+"_catastro.u_terreno where codigo = '"+codigo[0][0]+"'");
        document.getElementById("marco").style.display = 'block';
        document.getElementById("marco").src = "./fichas_prediales/"+mun+"/"+cod+".pdf";
    }
   else if(mun=='la_celia'){
...
function open_carta(gid){
    //console.log(gid);
    var cod = select_query("select codigo from "+mun+"_catastro.u_manzana where gid = "+gid+"");
    if(mun=='pueblo_rico'){
        var codcart = cod + "-22122020";
    }
    else if(mun=='apia'){
        var codcart = cod + "-28122020";
    }
    else if(mun=='belen_de_umbria'){
        var codcart = cod + "-28122020";
    }
    ...
    document.getElementById("marco").style.display = 'block';
    document.getElementById("marco").src = "./cartas_catastrales/"+mun+"/"+codcart+".pdf";
   // console.log(codigo);
}


function vistainicial(){
   var pgetextent = [-8435894.874793, 620451.145325, -8395913.163667, 642540.271079];
...
var opt = document.getElementById("gmapas").value;
    if(opt == 'sinseleccion'){
        alert("Por favor seleccione el mapa a consultar");
    }
    else if (opt == 'pc'){
        for (var b = 0; b< capasr.length; b++){
            capasr[b].values_.visible = false;
        }
        for(var j = 0; j< capasd.length; j++){
            if (capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_perimetro" || capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_manzana" || capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_terreno"  || capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_construccion"){
                capasd[j].values_.visible = true;         
            }     
            else{
                capasd[j].values_.visible = false;
            }
            vistainicial();
        }
    }
        ...
        for(var j = 0; j< capasd.length; j++){
            if (capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_zona_homogenea_fisica" || capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_manzana" || capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_perimetro"){
                capasd[j].values_.visible = true;         
            }     
            else{
                capasd[j].values_.visible = false;
        ...
        for(var j = 0; j< capasd.length; j++){
            if (capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_zona_homogenea_geoeconomica" || capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_manzana" || capasd[j].values_.source.params_.LAYERS == ""+mun+"_u_perimetro"){
                capasd[j].values_.visible = true;         
            }     
            else{
                capasd[j].values_.visible = false;
        ...
        for (var b = 0; b< capasr.length; b++){
            capasd[b].values_.visible = false;
        }
        for(var j = 0; j< capasr.length; j++){
            if (capasr[j].values_.source.params_.LAYERS == ""+mun+"_areas_de_amenaza" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_area_construida" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_perimetro" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_drenajes" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_vias" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_curvas_de_nivel" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_dtm"){
                capasr[j].values_.visible = true;         
            }     
            else{
                capasr[j].values_.visible = false;
            }
            if (capasr[j].values_.source.params_.LAYERS == ""+mun+"_dtm"){
                capasr[j].setOpacity(0.6);
            }
            vistainicial();
        }
    }
        ...
        for(var j = 0; j< capasr.length; j++){
            if (capasr[j].values_.source.params_.LAYERS == ""+mun+"_procesos_morfodinamicos" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_area_construida" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_perimetro" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_drenajes" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_vias" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_curvas_de_nivel" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_dtm"){
                capasr[j].values_.visible = true;         
            }     
            else{
                capasr[j].values_.visible = false;
        ...
        for(var j = 0; j< capasr.length; j++){
            if (capasr[j].values_.source.params_.LAYERS == ""+mun+"_pendientes" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_area_construida" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_perimetro" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_drenajes" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_vias" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_curvas_de_nivel" || capasr[j].values_.source.params_.LAYERS == ""+mun+"_dtm"){
                capasr[j].values_.visible = true;         
            }     
            else{
                capasr[j].values_.visible = false;
...
function descargarmapa(){
    ...
    var opt = document.getElementById("gmapas").value;
    document.getElementById("marco").style.display = 'block';
    if(opt == 'gestiondelriesgo'){
     document.getElementById("marco").src = 'pdfs/gestion_del_riesgo/'+plano+'.pdf';
    }
    else if(opt == 'pot'){
...
function cambiarcolor() {
	document.getElementById('button2').style.backgroundColor = '#ffc935';	
}	

function volvercolor() {
	document.getElementById('button2').style.backgroundColor = '#25a74f';	
}	
...
function modulocaptura(){
    window.open('http://www.ideamco.com/captura', '_blank');
}
function formulariocampo(){
    //window.open('http://34.66.84.60/formulario_amco/', '_blank');
    window.open('http://www.ideamco.com/totem/', '_blank');
}
...
function changeImage(id) {
   document.getElementById("boton_limpiar").style.background = "url('./imagenes/boton_limpiar_onmouseover.png'";
   document.getElementById("boton_limpiar").style.backgroundRepeat = "no-repeat";
}

function normalImage(id) {
   document.getElementById("boton_limpiar").style.background = "url('./imagenes/boton_limpiar.png'";
   document.getElementById("boton_limpiar").style.backgroundRepeat = "no-repeat";
}
...
var observaciones = document.getElementById('tipo_incidencia').value;
    if (observaciones == 'sin'){
        var incident = 'Sin Información';
        var estado = "No Aplica";
    }
    else if(observaciones == 'abierta'){
        var incident = 'INCIDENCIA';
        var estado = "ABIERTA";
    }
    else if(observaciones == 'cerrada'){
        var incident = 'INCIDENCIA';
        var estado = "CERRADA";
    }
   
var payload3 = '<wfs:Transaction service="WFS" version="1.0.0" xmlns:cimitarra="http://cimitarra.co" xmlns:ogc="http://www.opengis.net/ogc" xmlns:wfs="http://www.opengis.net/wfs"><wfs:Update typeName="cimitarra:terreno_rural"><wfs:Property><wfs:Name>incidencia</wfs:Name><wfs:Value>' + incident + '</wfs:Value></wfs:Property><wfs:Property><wfs:Name>estado</wfs:Name><wfs:Value>' + estado + '</wfs:Value></wfs:Property><ogc:Filter><ogc:PropertyIsEqualTo><ogc:PropertyName>objectid</ogc:PropertyName><ogc:Literal>' + objectid + '</ogc:Literal></ogc:PropertyIsEqualTo></ogc:Filter></wfs:Update></wfs:Transaction>';  
    
    
    
    
$.ajax('http://35.184.176.7:8081/geoserver/cimitarra/ows', {
        type: 'POST',
        dataType: 'xml',
        processData: false,
        contentType: 'text/xml',
    ...
    predio.getSource().updateParams({'STYLES': 'predios_rurales'});
    alert("Información guardada exitosamente</br>");
}


function menuplanospot(){
document.getElementById('mapaspot').style.display = 'block';
document.getElementById('carruspot').style.display = 'block';
}

function cerrarmenuplanos(){
document.getElementById('mapaspot').style.display = 'none';
document.getElementById('carruspot').style.display = 'none';
}

function planospot(id){
	document.getElementById('carruspot').style.display = 'none';
	document.getElementById('mapaspot').style.display = 'none';
	limpiar_consulta();
if (id == 'pot1') {
	...
	pot_clasificacion_suelo.setVisible(true);
	pot_suelo_suburbano.setVisible(true);
	}
	else if (id == 'pot2') {
	...
	pot_clasificacion_suelo.setVisible(true);
	pot_suelo_suburbano.setVisible(true);
	pot_areas_protegidas_sinap.setVisible(true);	
	}
	else if (id == 'pot3') {
	document.getElementById('l3').style.display = 'block';
	predio.setVisible(false);
	pot_amenaza_mov_masa.setVisible(true);
	}
}
...
function actlin(foto) {
 ...
 /*   var textx = 4113;
    ...
    var bus_text = search2("paicol:bus_text", foto);

    var textx1 = bus_text[0][0];
    var texty1 = bus_text[0][1];
    var textx2 = bus_text[0][2];
    var texty2 = bus_text[0][3];
    var textx3 = bus_text[0][4];
    var texty3 = bus_text[0][5];
...
/*id: 'flecha',
   ...
   /*   image: 'http://www.gesstor360.com/Paicol/imagenes/flecha_adelante.png',
      width: 36,
      height: 36,
...
function giropsv(giropsv) {
    ...
    var heading = parseFloat(fotopan2[0][5]);
...
function cerrarioptimize(){
    document.getElementById('marco3').style.display = 'none';
    document.getElementById('volveropt').style.display = 'none';
}
...
function datoscapas(datosc){
    ...
    // console.log(layer,ruta);


    // // console.log(idNombreLayer);
    // $('#'+idNombreLayer+'dc').removeClass('buttonDisable').addClass('buttonActive');
 
    var table = document.getElementById("tblatt");
    table.innerHTML = "";
    
    //$('#modal-title').text('DESCARGAR CAPA');
    var select = [];
    var sel = [];
    var sel2 = [];
    var rutalayer = JSON.stringify(ruta);
    select[0] = '<b>'+datosc+'</b>';             
    sel[0] = "<select class='form-control form-control-lg' style='font-size: 12px' select example' id='desc' onchange='descargarcapas("+rutalayer+")'><option value='descargar'>Descargar</option><option value='shape'>Shapefile</option><option value='kml'>Kml</option><option value='csv'>Hoja de Calculo</option><option value='geojson'>GeoJSON</option>";                           
    sel2[0] = "<button type='button' onclick='copiarwms("+rutalayer+")' class='btn btn-primary btn-sm'>Copiar WMS</button>";
...
function descargarcapas(datosc){
 
   var selectdesc = document.getElementById("desc").value;
   ...
   if (selectdesc == 'shape') {    
    mi_ventana.location.href = "http://35.232.57.213:8080/geoserver/buenaventura/ows?service=WFS&version=1.0.0&request=GetFeature&typeName="+datosc+"&outputFormat=SHAPE-ZIP";    
    
   }    
   else if (selectdesc == 'kml') {
     mi_ventana.location.href = "http://35.232.57.213:8080/geoserver/buenaventura/ows?service=WFS&version=1.0.0&request=GetFeature&typeName="+datosc+"&outputFormat=application/vnd.google-earth.kml+xml";   
     
   }
   else if (selectdesc == 'csv') {
     mi_ventana.location.href = "http://35.232.57.213:8080/geoserver/buenaventura/ows?service=WFS&version=1.0.0&request=GetFeature&typeName="+datosc+"&outputFormat=csv";   
     
   }
   else if (selectdesc == 'geojson') {
     mi_ventana.location.href = "http://35.232.57.213:8080/geoserver/buenaventura/ows?service=WFS&version=1.0.0&request=GetFeature&typeName="+datosc+"&outputFormat=application/json";   
    }
   sleep(20000);
   cierra_mi_ventana();
}
...
function enviarnuevacontrasena(){
    ...
    var useract = document.getElementById("labelusuario").innerHTML;
    if(newpass.length == 0){
        alert("Debe ingresar alguna contraseña");
    }
    else{
        var newpassaved = hex_md5(newpass);
        console.log(newpassaved);
        console.log(useract);
        update_query("update usuario set contrasena = '"+newpassaved+"' where usuario = '"+useract+"'");
...
function llenarlistmapas(){
    ...
    else if(document.getElementById("gmapas").value == 'gestiondelriesgo'){
     document.getElementById("seglist").style.display = 'block';
     var tb = select_query("select codigo, categoria, nombre_del_plano from mapas_pdf where categoria = 'Gestion del riesgo'");
   ...
   // var dat = validacionusuarios();
 //   console.log(dat);
   /* var select = document.getElementById("gmapas"); //Seleccionamos el select
    var tb = select_query("select codigo, nombre_del from mapas_pdf where municipio ='"+dat[0][7]+"'");
    //var tb = select_query("select codigo from mapas_pdf where municipio ='"+dat[0][7]+"'");
    for(var i=0; i < tb.length; i++){ 
        var option = document.createElement("option"); //Creamos la opcion
...
function openmanual(){
    document.getElementById("marco").style.display = 'block';
    document.getElementById("marco").src = "./pdfs/manual_de_usuario.pdf";
}
...
function openmdtpart(){ 
    /*document.getElementById('only2d').style.top = '55%';
    document.getElementById('only2d').style.display = 'block';
    document.getElementById('marco4').style.display = 'block';
    document.getElementById('marco4').style.top = '50%';
    document.getElementById('marco4').style.width = '100%';
    document.getElementById('marco4').style.height = '50%';*/
    document.getElementById('marco4').style.width = '100%';
    document.getElementById('marco4').style.height = '100%';
    document.getElementById('marco4').style.display = 'block';
